function varargout = periodicGradient(f,varargin)
%GRADIENT Approximate gradient on a ring (without boundary conditions).
...
%   direction. The spacing between points in each direction, as
%   well as the spacing between the last and the first point in
%   each direction, is assumed to be one. When F is a vector, DF =
%   GRADIENT(F) is the 1-D gradient.
%
...
elseif ndim > 2
    ...
    for k = 2:ndim
        ...
        if n > 2
            h = h(3:n) - h(1:n-2);
            g(:,2:n-1,:) = bsxfun(@rdivide,(nf(:,3:n,:)-nf(:,1:n-2,:)),h);
        end
